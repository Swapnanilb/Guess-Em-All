...
export default function Header({ view, setView, resetProgress, user, onLogout }) {
  return (
    <header className="sticky top-0 z-10 backdrop-blur supports-[backdrop-filter]:bg-black/40 bg-black/30 border-b border-white/10">
      <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {onLogout && (
            <button onClick={onLogout} className="px-3 py-2 rounded-xl border border-white/10 hover:border-white/30 bg-zinc-900/40">
              Logout
            </button>
          )}
        </div>
